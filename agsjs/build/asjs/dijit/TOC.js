/*built on 2012-08-21 14:32:39.70*/
...
dojo.ready(function () {
	dojo.declare("agsjs.dijit._TOCNode", [dijit._Widget, dijit._Templated], {
		...
		postCreate : function () {
			...
			if (this.legend) {
				...
				this.rootLayer && this._createRootLayerNode
				(this.rootLayer);
			}
			...
			if (this.data._subLayerInfos) {
				var b = true;
				dojo.every(this.data._subLayerInfos,
					function (d) {
					if (d.visible)
						return b = false;
					return true
			...
			if (this.containerNode)
				dojo.style(this.containerNode, "display", a ? "block" : "none");
		...
		_createRootLayerNode : function (a) {
			...
			var b = this.rootLayerTOC.info.title;
			if (b === "") {
				esri.hide(this.rowNode);
				a.show();
				this.rootLayerTOC._currentIndent--
			}
			a.collapsed = this.rootLayerTOC.info.collapsed;
			if (this.rootLayerTOC.info.slider) {
				...
				dojo.connect(a, "onOpacityChange", this, function (d) {
					this.slider.setValue(d * 100)
				})
			}
			this.rootLayerTOC.info.noLegend ? dojo.style(this.iconNode,

				"visibility", "hidden") : this._createChildrenNodes(a._tocInfos, "layer");
			this.labelNode.innerHTML = b;
			dojo.attr(this.rowNode, "title", b)
		},
		_createLayerNode : function (a) {
			this.labelNode.innerHTML = a.name;
			if (a._subLayerInfos) {
				dojo.addClass(this.rowNode,
					"agsjsTOCGroupLayer");
				dojo.addClass(this.labelNode, "agsjsTOCGroupLayerLabel");
				if (this.rootLayerTOC.info.showGroupCount)
					this.labelNode.innerHTML = a.name + " (" +

						a._subLayerInfos.length + ")";
				this._createChildrenNodes(a._subLayerInfos, "layer")
			} else {
				dojo.addClass(this.rowNode, "agsjsTOCLayer");
				dojo.addClass(this.labelNode, "agsjsTOCLayerLabel");
				if (this.rootLayer.visibleLayers)
					a.visible = dojo.indexOf

						(this.rootLayer.visibleLayers, a.id) == -1 ? false : true;
				if (this.rootLayer.tileInfo)
					this._noCheckNode = true;
				if (a.renderer &&
					!this.rootLayerTOC.info.noLegend &&

					this.rootLayerTOC.info.mode != "legend")
					if (this.rootLayerTOC.toc.style == "inline" &&

						a.renderer.symbol) {
						this._setIconNode(a.renderer, this.iconNode,

							this);
						dojo.destroy(this.containerNode);
						this.containerNode = null
					} else if (a.renderer instanceof

						esri.renderer.SimpleRenderer)
						this._createChildrenNodes([a.renderer],

							"legend");
					else {
						var b = a.renderer.infos;
						if (a.renderer.defaultSymbol)
							b = [{
									symbol :

									a.renderer.defaultSymbol,
									label :

									a.renderer.defaultLabel,
									isDefault : true,
									url :
									...
									a.renderer.imageData,
									value : "*"
								}
							].concat(b);
						this._createChildrenNodes(b, "legend")
					}
				else if (a.legends && !this.rootLayerTOC.info.noLegend)
					if (this.rootLayerTOC.toc.style == "inline" &&

						a.legends.length == 1) {
						this.iconNode.src = this._getLegendIconUrl
						...
						this._createChildrenNodes(a.legends,
		...
		_createLegendNode : function (a) {
			...
			if (this.layer && this.layer.definitionExpression)
				if (this.layer.renderer && this.layer.renderer instanceof esri.renderer.UniqueValueRenderer && !this.layer.renderer.attributeField2)
					if (!a.isDefault && this.rootLayerTOC.toc.style == "inline" && this.rootLayerTOC.info.mode != "legend")
						a.visible = true;
			if (!a.visible)
				this._noCheckNode = true;
			dojo.destroy(this.containerNode);
			...
			this.labelNode.appendChild(document.createTextNode(a.label));
		},
		_setIconNode : function (a, b, d) {
			var c = this._getLegendIconUrl(a);
			if (!c || this.rootLayerTOC.info.mode == "layers") {
				if (a.symbol) {
					a = this._createSymbol(a.symbol);
					dojo.place(a, b, "replace");
					d.iconNode = a;
				}
			} else {b.src = c;}
		},
		_createSymbol : function (a) {
			var b = this.rootLayerTOC.toc.swatchSize[0],
			d = this.rootLayerTOC.toc.swatchSize[1];
			if (a.width && a.height) {
				b = a.width;
				d = a.height
			}
			...
			e = dojox.gfx.createSurface(c, b, d);
			(a = esri.symbol.getShapeDescriptors(a)) && e.createShape

			(a.defaultShape).setFill(a.fill).setStroke(a.stroke).applyTransform({
				dx : b /
				2,
				dy : d / 2
			});
		...
		_getLegendIconUrl : function (a) {
			...
			if (b != null && b.indexOf("data") == -1)
				if (!dojo.isIE && a.imageData && a.imageData.length > 0)
					...
					b = this.rootLayer.url + "/" + this.layer.id +
		...
		_adjustToState : function () {
			if (this.checkNode) {
				var a = this.legend ? this.legend.visible : this.layer ?
					this.layer.visible : this.rootLayer ? this.rootLayer.visible : false;
				if (this.checkNode.set)
					this.checkNode.set("checked", a);
				else
					this.checkNode.checked = a
			}
			if (this.layer) {
				a = esri.geometry.getScale(this.rootLayerTOC.toc.map);
				(a = this.layer.maxScale != 0 && a < this.layer.maxScale ||

						this.layer.minScale != 0 && a > this.layer.minScale) ? dojo.addClass(this.domNode,

					"agsjsTOCOutOfScale") : dojo.removeClass(this.domNode, "agsjsTOCOutOfScale");
				if (this.checkNode)
					if (this.checkNode.set)
						this.checkNode.set("disabled", a);
					else
						this.checkNode.disabled = a
			}
			this._childTOCNodes.length > 0 && dojo.forEach(this._childTOCNodes,
				function (b) {
				b._adjustToState()
			})
		},
		_onClick : function (a) {
			a = a.target;
			if (a == this.checkNode || dijit.getEnclosingWidget(a) ==
				...
				if (this.legend) {
					...
					var d = [];
					if (b instanceof esri.renderer.UniqueValueRenderer)
						{
						var c = "";
						this.layer.fields && dojo.forEach

						(this.layer.fields, function (e) {
							if (e.name.toLowerCase() ==

								b.attributeField.toLowerCase())
								switch (e.type) {
								case "esriFieldTypeString":
									c = "'"
								}
						});
						dojo.forEach(b.infos, function (e) {
							e.visible && d.push(c + e.value + c)
						}, this);
						if (d.length == b.infos.length)
							...
							this.layer._definitionExpression =
								...
								b.attributeField + " IN (" + d.join(",") + ")"
						}
					}
					this.rootLayer.setVisibleLayers

					(this._getVisibleLayers(), true);
					this.rootLayer.setLayerDefinitions

					(this._getLayerDefs(), true);
					this.rootLayerTOC._refreshLayer()
				} else if (this.layer) {
					this.layer.visible = this.checkNode &&

						this.checkNode.checked;
					if (this.layer._parentLayerInfo && !

						this.layer._parentLayerInfo.visible)
						this.layer._parentLayerInfo.visible = true;
					this.layer.visible && !this.rootLayer.visible &&

					this.rootLayer.show();
					this.layer._subLayerInfos &&
					dojo.forEach(this.layer._subLayerInfos, function (e)
						{
						e.visible = this.layer.visible
					}, this);
					if (this.layer.renderer) {
						dojo.forEach(this.layer.renderer.infos,
					...
					this.rootLayer && this.rootLayer.setVisibility

					(this.checkNode && this.checkNode.checked);
				//this.rootLayerTOC.toc.style == "inline" && this._toggleContainer(this.checkNode && this.checkNode.checked);
		...
		_getVisibleLayers : function () {
			var a = [];
			dojo.forEach(this.rootLayer.layerInfos, function (b) {
		...
		_getLayerDefs : function () {
			var a = [];
			dojo.forEach(this.rootLayer.layerInfos, function (b, d) {
				if (b._definitionExpression)
					a[d] = b._definitionExpression
			});
			return a
		}
	});
	dojo.declare("agsjs.dijit._RootLayerTOC", [dijit._Widget], {
		...
		_getLayerInfos : function () {
			if (this.rootLayer instanceof
				esri.layers.ArcGISDynamicMapServiceLayer || this.rootLayer instanceof
				esri.layers.ArcGISTiledMapServiceLayer) {
				if (this.info.title === undefined) {
					var a = this.rootLayer.url.toLowerCase().indexOf
						("/rest/services/"),
					b = this.rootLayer.url.toLowerCase().indexOf
						("/mapserver", a);
					this.info.title = this.rootLayer.url.substring(a +
							15, b)
				}
			} else
		...
		_processLegendInfo : function (a) {
			this.rootLayer._legendResponse = a;
			if (this.info.mode == "legend" || this.rootLayer._layersResponse)
				this._createRootLayerTOC()
		},
		_processLayersInfo : function (a) {
			this.rootLayer._layersResponse = a;
		...
		_createRootLayerTOC : function () {
			...
			if (!a._tocInfos) {
				var b = {};
				dojo.forEach(a.layerInfos,
					function (c) {
					b["" + c.id] = c;
					c.visible = c.defaultVisibility
				});
				a._layersResponse && dojo.forEach(a._layersResponse.layers,
					function (c) {
					var e = b["" + c.id];
					if (e) {
						dojo.mixin(e, c);
						e.definitionExpression = c.definitionExpression;
						if (c.drawingInfo && c.drawingInfo.renderer)
							e.renderer = esri.renderer.fromJson
								(c.drawingInfo.renderer)
					}
				});
				a._legendResponse && dojo.forEach(a._legendResponse.layers,
					function (c) {
					var e = b["" + c.layerId];
					if (e && c.legend) {
						e.legends = c.legend;
						if (e.renderer)
							if (e.renderer.infos) {
								var g = {};
								dojo.forEach(c.legend,
									function (f) {
									g[f.label] = f
								});
								e.renderer.defaultSymbol &&
								dojo.mixin(e.renderer, c.legend[0]);
								dojo.forEach
								(e.renderer.infos, function (f) {
									var h = g[f.label];
									h && dojo.mixin(f,
										h)
								})
							} else
								dojo.mixin(e.renderer,
									c.legend[0])
					}
				});
				dojo.forEach(a.layerInfos, function (c) {
					if (c.subLayerIds) {
						var e = [];
						dojo.forEach(c.subLayerIds, function (g, f)
							{
							e[f] = b[g];
							e[f]._parentLayerInfo = c
						});
						c._subLayerInfos = e
					}
				});
			...
			this._visHandler = dojo.connect(a, "onVisibilityChange", this,
					"_adjustToState");
			this._visLayerHandler = dojo.connect(a, "setVisibleLayers", this,
					"_onSetVisibleLayers");
			this._adjustToState()
		},
		...
		_onSetVisibleLayers : function (a, b) {
			if (!b) {
				dojo.forEach(this.rootLayer.layerInfos,
					function (d) {
					if (dojo.indexOf(a, d.id) != -1)
						d.visible = true;
					else if (!d._subLayerInfos)
						d.visible = false
				});
				this._adjustToState()
			}
	...
	dojo.declare("agsjs.dijit.TOC", [dijit._Widget], {
		...
		constructor : function (a) {
			...
			if (!this.layerInfos) {
				...
				for (a = this.map.layerIds.length - 1; a >= 0; a--) {
					...
					!b._isBaseMap && !b._isReference &&
					this.layerInfos.push({
						layer : b
					})
				}
				dojo.connect(this.map, "onLayerAdd", this, function (d) {
					this.layerInfos.push({
						layer : d
					});
					this.refresh()
				});
				dojo.connect(this.map, "onLayerRemove", this, function (d) {
					for (var c = 0; c < this.layerInfos.length; c++)
						if (this.layerInfos[c] == d) {
		...
		postCreate : function () {
			var a = [];
			dojo.forEach(this.layerInfos, function (d) {
				if (!d.layer) {
					d.layer = this._createRootLayer(d);
					a.push(d.layer)
				}
			}, this);
			if (a.length == 0)
				...
				var b = dojo.connect(this.map, "onLayersAddResult", this,
						function () {
						dojo.disconnect(b);
						this._createTOC()
					});
		...
		onLoad : function () {},
		_createRootLayer : function (a) {
			var b = null;
			switch (a.type || "ArcGISDynamic") {
			case "ArcGISDynamic":
				b = new esri.layers.ArcGISDynamicMapServiceLayer(a.url, a);
				break
			}
			return b
		},
		_createTOC : function () {
			...
			for (var a = 0, b = this.layerInfos.length; a < b; a++) {
				var d = new agsjs.dijit._RootLayerTOC({
						rootLayer : this.layerInfos[a].layer,
						info : this.layerInfos[a],
						toc : this
					});
		...
		_adjustToState : function () {
			dojo.forEach(this._rootLayerTOCs, function (a) {
				a._adjustToState();
			})
		},
		refresh : function () {
